...
mod tests {
    ...
    fn test_purchase_insurance_success() {
        ...
        let token_admin = Address::generate(&env);
        let (_, token_client, token_admin_client) = create_token(&env, &token_admin);
        let sponsor = Address::generate(&env);
        token_admin_client.mint(&sponsor, &10_000);

        client.purchase_insurance(&sponsor, &1, &5_000, &token_addr);
    ...
    fn test_claim_insurance_success() {
        ...
        let token_admin = Address::generate(&env);
        let (_, _, token_admin_client) = create_token(&env, &token_admin);
        let sponsor = Address::generate(&env);
        token_admin_client.mint(&sponsor, &10_000);
